  ...
  ({
    ...
    "cdn.js": () => {
      ...
      build({
        ...
        outputSize(
          pkg,
          `packages/${pkg}/dist/${file.replace(".js", ".min.js")}`
        );
      });
    },
...
function build(options) {
  ...
  options.define["MYLIB_VERSION"] = `'${getFromPackageDotJson(
    "core",
    "version"
  )}'`;
...
function writeToPackageDotJson(pkg, key, value) {
  let dotJson = new DotJson(`./packages/${pkg}/package.json`);

  dotJson.set(key, value).save();
}

function getFromPackageDotJson(pkg, key) {
  let dotJson = new DotJson(`./packages/${pkg}/package.json`);
